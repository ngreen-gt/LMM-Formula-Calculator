/*
 ...
 HR2.C
 V1.03 (Last updated by NG from 1.02 --> 1.03)
 ...
 *added extended valencies, added implementation of
  ...
  *changed version by Nelson Green (NG), 2014, in E. Micheal Perdue's lab at Ball State
  *implemented core CHO calculation, which decrease loops to number of elements (N) - 3
  *correspond with E. Michael Perdue at emperdue@bsu.edu
...
--------------------------------------------------------------------
 ...
 Revision:  2001-04-18, GPL'd, first public release (JHa)
			...
			2014-01-30, added CHO calculation in loop core, removing loops and increasing speed (NG)
 ...
 For compiling I'm using Visual C++ 2012 Express in C++ mode with /Ox full optimization (NG).
...
bool    calc_core(double core_mass, int core_nm);
int     clean (char *buf);
...
/**********************************************************************
*  CUSTOM_ROUND:  Calculates normal round using ceil and floor. (NG)
*  Input: float x
*  Return: int x
*  Notes: Round not available in math.h, but if it becomes available change to it.
***********************************************************************/
int custom_round(double x)
{
if (x > 0)
...
/************************************************************************
*  CALC_CORE:  Calculate core CHO molecular formula
*  Input: double core_mass, int core_nm 
*  Returns: boolean true/false for found/notfound CHO core
*  Notes: Searches through H/C vs O/C diagram at fixed nominal mass, where
*         only a certain number of CHO molecules can exist, and all lay on
*         lines that radiate from (4, -1) equivalent to CH4 - O.  The maximum
*         number of these lines is defined by the empirical formula 
*         MaxAttempts = Round (NM/60 + 0.8) that was determined using all 
*         terates to correct CHO value ***may need to reset CHO manually
*         Will only output first 
***************************************************************************/

bool calc_core (double core_mass, int core_nm)
{
double limit_lo, limit_hi; /* mass limits */
int moiety;				   /* # of CH4-O moiety (+/-) needed to minimimize mass  */
double core_calc_mass;     /* calculated mass of CHO core molecule */
char attempts;             /* count of C4 - O3 moieties applied to minimum hydrocarbon formula */
char max_attempts;         /* empiricial limit on number of C4-O3 moieties to be applied to find valid formula */

/* Note: Max_attempts was empirically determined by counting the number of attempts needed to find each of 53573 CHO-only
*  formula that were considered valid between 150 and 1000 Daltons.  However, the valid determination used is different
*  than the one in HR2.c, and so there may be a slight different because of this.  I have not fixed this but to fix this
*  a complete set of valid CHO formula for a large mass range should be made, and using hard limits the attempts to find
*  the correct solution can be determined.  From this a new empirical relation may be found, but for now I will use the old relation */

const double moietymass = 0.0363855087200022;  
core_calc_mass=0;
limit_lo = core_mass - (tol / 1000.0);
limit_hi = core_mass + (tol / 1000.0);

if (core_nm % 2==0)
{
el[0].cnt=core_nm / 12;  //integer division of NM by 12 (carbon nominal mass)
el[2].cnt=core_nm-12*el[0].cnt;  //subtract 12*C to find H for minimum hydrocarbon
el[6].cnt=0;

core_calc_mass = el[0].mass*el[0].cnt+el[2].mass*el[2].cnt+el[6].mass*el[6].cnt;
attempts=0;
max_attempts=custom_round(0.8+core_nm/60);

while (((core_calc_mass<limit_lo) || (core_calc_mass>limit_hi)) && (attempts<max_attempts))
	{
	moiety=custom_round((core_mass-core_calc_mass)/moietymass);
	if ((core_calc_mass+moiety*moietymass>=limit_lo) && (core_calc_mass+moiety*moietymass<=limit_hi))
		{
		if ((el[0].cnt+moiety>=1) && (el[2].cnt+4*moiety>=1) &&(el[6].cnt-moiety>=0))
			{
			el[0].cnt=el[0].cnt+moiety;
			el[2].cnt=el[2].cnt+4*moiety;
			el[6].cnt=el[6].cnt-moiety;
			}
		else
			{
...
* (NG) Notes: Removed CHO loops, replaced with core_calc.  
*************************************************************************/
...
//while (el[2].cnt++ < el[2].max) /*"H"*/{ 
	...
	if (core_mass>el[0].mass+4*el[2].mass)
		{
		core_nm=custom_round(core_mass);
		if (core_nm%2==0)
			{
			formulaOK=calc_core(core_mass,core_nm);
			}
		else
			{
			if (core_mass>=31*el[0].mass+64*el[2].mass)
				{
				formulaOK=calc_core(core_mass, core_nm-1);
				}
			}
		}
	counter++;
